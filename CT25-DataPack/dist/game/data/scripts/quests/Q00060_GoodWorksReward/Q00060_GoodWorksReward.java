...
public final class Q00060_GoodWorksReward extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "You have good luck. I shall return."));
			...
			if (qs.isMemoState(1))
			{
				if (killer.isPlayer())
				{
					if (killer.getObjectId() == npc.getVariables().getInt("PLAYER_ID", 0))
					{
						qs.setMemoState(2);
						qs.setCond(2, true);
						npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "You are strong. This was a mistake."));
					}
					else
					{
						npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "Who are you to join in the battle? How upsetting."));
					}
				}
			}
			final L2Npc npc0 = npc.getVariables().getObject("npc0", L2Npc.class);
	...
	public String onSpawn(L2Npc npc)
	{
		...
		if (player != null)
		{
			if (player.isPlayer())
			{
				npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "$s1! I must kill you. Blame your own curiosity.").addStringParameter(player.getAppearance().getVisibleName()));
			}
		}
