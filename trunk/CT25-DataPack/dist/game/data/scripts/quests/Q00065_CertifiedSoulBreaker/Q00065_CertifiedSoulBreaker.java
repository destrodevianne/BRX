...
public final class Q00065_CertifiedSoulBreaker extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if ("DESPAWN_70".equals(event))
		{
			...
			if (npc0 != null)
			{
				if (npc0.getVariables().getBoolean("SPAWNED"))
				{
					...
					if (c0 != null)
					{
						npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "$s1! I will be back soon. Stay there and don't you dare wander off!").addStringParameter(c0.getAppearance().getVisibleName()));
					}
				}
			}
			npc.deleteMe();
		...
		switch (event)
		{
			...
			case "32213-04.htm":
			{
				if ((player.getLevel() >= MIN_LEVEL) && (player.getClassId() != ClassId.trooper || player.getClassId() != ClassId.warder))
				{
					htmltext = event;
				}
				break;
			...
			switch (npc.getId())
			{
				...
				case GUARDIAN_ANGEL:
				{
					...
					if (killer == c0)
					{
						if (c0 != null)
						{
							if (qs.isMemoState(12))
							{
								...
								qs.setMemoState(13);
								npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "Grr. I've been hit..."));
							}
						}
					}
					else
					{
						...
						npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "Grr! Who are you and why have you stopped me?"));
					}
				}
			}
		}
	...
	public String onSpawn(L2Npc npc)
	{
		if (npc.getId() == SUSPICIOUS_MAN)
		{
			...
			npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "Drats! How could I be so wrong??"));
		...
		else if (npc.getId() == GUARDIAN_ANGEL)
		{
			...
			if (c0 != null)
			{
				npc.broadcastPacket(new NpcSay(npc, Say2.NPC_ALL, "$s1! Step back from the confounded box! I will take it myself!").addStringParameter(c0.getAppearance().getVisibleName()));
			}
		}
	...
	{
		new Q00065_CertifiedSoulBreaker();
	}
}