...
public class Q270_TheOneWhoEndsSilence extends Quest
{
	...
	//Kill Ids
	private static final int[] KILL_IDS = { SEEKER_SOLINA, SAVIOR_SOLINA, ASCETIC_SOLINA, DIVINITY_JUDGE, DIVINITY_MANAGER, DIVINITY_SUPERVISOR,
	...
	public Q270_TheOneWhoEndsSilence(int questId, String name, String descr)
	{
		...
		for (int mobId : KILL_IDS)
			addKillId(mobId);
		
		questItemIds = new int[]{TATTERED_MONK_CLOTHES};
	}
	...
	private static void giveItem(L2PcInstance player, L2Npc npc, int chance, boolean atLeastOne)
	{
		...
		{
			final int count = ((Rnd.get(1000) < chance) ? 1 : 0) + (atLeastOne ? 1 : 0);
			if (count > 0)
			{
				final QuestState qs = player.getQuestState(Q270_TheOneWhoEndsSilence.class.getSimpleName());
	...
	{
		new Q270_TheOneWhoEndsSilence(270, Q270_TheOneWhoEndsSilence.class.getSimpleName(), "The One Who Ends Silence");
	}
}
