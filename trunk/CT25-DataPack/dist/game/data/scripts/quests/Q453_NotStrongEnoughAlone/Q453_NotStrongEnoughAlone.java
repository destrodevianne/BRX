...
public class Q453_NotStrongEnoughAlone extends Quest
{
	...
	private void increaseNpcKill(L2PcInstance player, L2Npc npc)
	{
		...
		if (Util.contains(MONSTER1, npc.getNpcId()) && st.getInt("cond") == 2)
		{
			int val = 0;
			
			if (npc.getNpcId() == MONSTER1[0] || npc.getNpcId() == MONSTER1[4])
				val = MONSTER1[0];
			else if (npc.getNpcId() == MONSTER1[1] || npc.getNpcId() == MONSTER1[5])
				val = MONSTER1[1];
			else if (npc.getNpcId() == MONSTER1[2] || npc.getNpcId() == MONSTER1[6])
				val = MONSTER1[2];
			else if (npc.getNpcId() == MONSTER1[3] || npc.getNpcId() == MONSTER1[7])
				val = MONSTER1[3];
		...
		else if (Util.contains(MONSTER2, npc.getNpcId()) && st.getInt("cond") == 3)
		{
			int val = 0;
			
			if (npc.getNpcId() == MONSTER2[0] || npc.getNpcId() == MONSTER2[3])
				val = MONSTER2[0];
			else if (npc.getNpcId() == MONSTER2[1] || npc.getNpcId() == MONSTER2[4])
				val = MONSTER2[1];
			else if (npc.getNpcId() == MONSTER2[2] || npc.getNpcId() == MONSTER2[5])
				val = MONSTER2[2];
		...
		else if (Util.contains(MONSTER3, npc.getNpcId()) && st.getInt("cond") == 4)
		{
			int val = 0;
			
			if (npc.getNpcId() == MONSTER3[0] || npc.getNpcId() == MONSTER3[3])
				val = MONSTER3[0];
			else if (npc.getNpcId() == MONSTER3[1] || npc.getNpcId() == MONSTER3[4])
				val = MONSTER3[1];
			else if (npc.getNpcId() == MONSTER3[2] || npc.getNpcId() == MONSTER3[5])
				val = MONSTER3[2];
	...
	{
		new Q453_NotStrongEnoughAlone(453, Q453_NotStrongEnoughAlone.class.getSimpleName(), "Not Strong Enought Alone");
	}
}
