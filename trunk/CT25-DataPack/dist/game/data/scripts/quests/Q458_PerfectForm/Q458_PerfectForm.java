...
public class Q458_PerfectForm extends Quest
{
	...
	private Q458_PerfectForm()
	{
		super(458, Q458_PerfectForm.class.getSimpleName(), "Perfect Form");
		addStartNpc(KELLEYIA);
		addTalkId(KELLEYIA);
		
		for (int i : KOOKABURRAS)
		{
			addKillId(i);
		}
		for (int i : COUGARS)
		...
		switch (event)
		{
			...
			case "32768-17.html":
				if (st.isCond(2))
				{
					...
					if (overhitsConsecutive >= 20)
					{
						int rnd = Rnd.get(ICARUS_WEAPON_RECIPES.length);
						st.rewardItems(ICARUS_WEAPON_RECIPES[rnd], 1);
					}
					else if (overhitsConsecutive >= 7)
					{
						int rnd = Rnd.get(ICARUS_WEAPON_PIECES.length);
